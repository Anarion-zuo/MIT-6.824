...
func TestReElection2A(t *testing.T) {
	...
	fmt.Printf("tester: disconnect leader %d\n", leader1)
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	fmt.Printf("tester: reconnect leader %d...\n", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	fmt.Printf("tester: disconnect leader %d along with follower %d...\n", leader2, (leader2+1)%servers)
	...
	fmt.Printf("tester: connect %d...\n", (leader2+1)%servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	fmt.Printf("tester: connect old leader %d...\n", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
...
func TestManyElections2A(t *testing.T) {
	...
	for ii := 1; ii < iters; ii++ {
		...
		i2 := rand.Int() % servers
		i3 := rand.Int() % servers
		fmt.Printf("test iter %d: disconnect 3 hosts %d %d %d\n", ii, i1, i2, i3)
		cfg.disconnect(i1)
		cfg.disconnect(i2)
		cfg.disconnect(i3)
		...
		fmt.Printf("test iter %d: the old leader should be alive\n", ii)
		cfg.checkOneLeader()

		fmt.Printf("test iter %d: reconnect servers %d %d %d\n", ii, i1, i2, i3)
		cfg.connect(i1)
		cfg.connect(i2)
		cfg.connect(i3)
	}
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			log.Printf("tester: disconnect victim %d", victim)
			cfg.disconnect(victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		if crash {
			log.Printf("tester: crash victim %d", victim)
			cfg.crash1(victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		...
		if disconnect {
			...
			log.Printf("tester: reconnect victim %d", victim)
			cfg.connect(victim)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
		if crash {
			log.Printf("tester: restart victim %d", victim)
